...
public class LevelManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject[] TPingPrefabs;
    ...
    // 리미트 초기화 값
    [SerializeField] private TextMeshProUGUI DragPointText;

    // 인게임 팝업
    [Header("인게임 팝업")] [SerializeField] private GameObject[] InGamePopUps;
    // 0: 리마인드, 1: 게임오버, 2: 클리어, 3: Pause메뉴 4: 종료확인

    // 인게임 팝업 배경
    [SerializeField] private GameObject InGamePopUpBG;

    [Header("목표(상단바)")] [SerializeField] private GameObject[] Targets_Top;

    [Header("목표(리마인드)")] [SerializeField] private GameObject[] Targets_Remind;

    [Header("목표 값")] [SerializeField] private int[] targetIndex = new int[6];
    ...
    // 전체 티니핑 관리 배열
    [Header("전체 블록")] public List<Tping> _allTpings = new List<Tping>();

    // 티니핑 선택 배열
    [Header("선택된 블록")] public List<Tping> _selectPings = new List<Tping>();

    [Header("파괴되는 블록")] public List<Tping> _erasePings = new List<Tping>();

    // 선택된 티니핑 ID
    [HideInInspector] public int _selectedID;

    // 플레이 체크
    [HideInInspector] public bool _isPlaying;

    // 제한 횟수
    [HideInInspector] public int DragPoint;
