...
public class BackendGameData
{
    ...
    public void GameDataInsert() {
        ...
        userData.clearedStage = 1;
        userData.heart = 5;
        userData.coin = 0;

        userData.items.Add("Move3", 5);
        userData.items.Add("Pop", 5);
        userData.items.Add("Shake", 3);

        userData.collection.Add("하츄핑");

        Debug.Log("뒤끝 업데이트 목록에 해당 데이터들을 추가합니다.");
        Param param = new Param();
        param.Add("clearedStage", userData.clearedStage);
        param.Add("heart", userData.heart);
    ...
    public void GameDataGet() {
        ...
        if (bro.IsSuccess()) {
            ...
            if (gameDataJson.Count <= 0) {
                ...
                userData.clearedStage = int.Parse(gameDataJson[0]["clearedStage"].ToString());
                userData.heart        = int.Parse(gameDataJson[0]["heart"].ToString());
